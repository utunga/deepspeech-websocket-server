...
try:
    ...
    def callback(indata, frames, time, status):
        ...
        if any(indata):
            ...
            bytes_arr = np.chararray.tostring(indata.astype(np.int16))
            is_speech = vad.is_speech(bytes_arr, samplerate)

            #if is_speech:
                
            print(*(1 if is_speech else 0 for x in range(args.columns)), sep='')
        else:
            print('no input')
